...
import io.github.ititus.steam_api.remote_storage.PublishedFileDetails;
...
public final class WingmanFinder {
    ...
    public static void main(String[] args) throws Exception {
        SteamInstallation steam = SteamInstallation.find();
        Path mapDirPath = steam.getGameDir("Counter-Strike Global Offensive").resolve(Path.of("csgo", "maps"));

        RemoteStorage remoteStorage = SteamWebApi.create().remoteStorage();

        MapDirectory mapDir = new MapDirectory(mapDirPath);
    ...
    private static <T extends Comparable<? super T>> void printSorted(List<Pair<MapInfo, PublishedFileDetails>> workshopMaps,
                                                                      Function<PublishedFileDetails, ? extends T> sorter, boolean reversed) {
